...
from MetricsFeeder.src.pipelines import field_translator, csv_to_json, json_to_TSDB_json
...
def bad_finish(error):
    eprint("[NETHOGS TO JSON] error in : " + str(error))
    sys.exit(1)
...
def process_line(line, metrics_dict, tags_dict, template):
    ...
    if line_in_csv:
        ...
        for json in csv_to_json.process_line(line_translated, metrics_dict, tags_dict, template):
